...
const BLOCK_COLORS = {
  rupture: {
    ...
    bgStrong: "rgba(226,75,74,0.16)",
    border: "rgba(226,75,74,0.2)",
    text: "#A32D2D",
  },
  stock_faible: {
    bg: "rgba(239,159,39,0.08)",
    bgStrong: "rgba(239,159,39,0.16)",
    border: "rgba(239,159,39,0.2)",
    text: "#BA7517",
  },
  stock_ok: {
    bg: "rgba(0,0,0,0.03)",
    bgStrong: "rgba(0,0,0,0.03)",
    border: "transparent",
    text: "inherit",
  },
  surstock: {
...
export function CarteRecipeCard({
  ...
  const isStockFaible =
    portionInfo.maxPortions > 0 && portionInfo.maxPortions < 10
  ...
  const limitingSupplier = limitingProduct
    ...
    : null
  const limitingStatus = limitingProduct
    ? getProductStatus(limitingProduct)
    : null

  const barPercent = Math.min(
    100,
    (portionInfo.maxPortions / PORTION_REFERENCE) * 100
  )
  const barFillColor = isRupture
    ? "#E24B4A"
    : isStockFaible
      ? "#EF9F27"
      : "#97C459"
  ...
  const portionTextColor = isRupture
    ? "#A32D2D"
    : isStockFaible
      ? "#BA7517"
      : "#639922"
  ...
  // Operational view: hide card if a product is hovered and this recipe doesn't use it
  const usesHoveredProduct = opView.isEditing && opView.hoveredProductId
    ? recipe.ingredients.some((i) => i.productId === opView.hoveredProductId)
    : null // null = no product hovered
  const isHidden = usesHoveredProduct === false
  const isHighlighted = usesHoveredProduct === true
  ...
  return (
    <motion.div
      ...
      onMouseEnter={() =>
        opView.isEditing && opView.setHoveredRecipe(recipe.id, recipe.name)
      }
      onMouseLeave={() =>
        opView.isEditing && opView.setHoveredRecipe(null)
      }
    >
      <Card
        role="article"
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter") onClick(recipe.id)
        }}
      >
        {/* Header: Name + Price/FC + Portions */}
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <h3 className="text-[15px] leading-[1.3] font-medium">
              {recipe.name}
            </h3>
            <div className="mt-px flex items-center gap-1.5">
              <span className="text-[11px] text-muted-foreground">
                ...
                <span
                  className={cn(
                    "font-medium",
                    getFoodCostColor(portionInfo.foodCostPercent)
                  )}
                >
        ...
        {/* Bottom row: limit text (left) + expand toggle (right) */}
        <div className="mt-1.5 flex items-center justify-between gap-2">
          {hasProblems && portionInfo.limitingIngredient ? (
            <p
              ...
              style={{
                color:
                  limitingStatus === "rupture"
                    ? "#A32D2D"
                    : limitingStatus === "stock_faible"
                      ? "#BA7517"
                      : undefined,
              }}
            >
              {isRupture ? "Rupture" : "Limité par"} :{" "}
              {portionInfo.limitingIngredient.productName}
          ...
          {totalIngredients > 0 && (
            <button
              ...
              aria-label={expanded ? "Masquer les ingrédients" : "Voir les ingrédients"}
              onClick={(e) => {
                e.stopPropagation()
...
function IngredientBlock({
  ...
  return (
    <div
      className="flex cursor-pointer items-center gap-2 rounded-lg hover:ring-1 hover:ring-border transition-shadow"
      style={{
        padding: "5px 7px",
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <p className="text-[10px] leading-none text-muted-foreground">
          {d.portionsAllowed} port. · {product.quantity}{" "}
          {UNIT_LABELS[product.unit]} dispo
        </p>
      </div>
    </div>
  )
...
function IngredientChip({
  ...
  return (
    <span
      className="inline-flex cursor-pointer items-center gap-1.5 rounded-md bg-secondary px-2 py-0.5 hover:ring-1 hover:ring-border transition-shadow"
      onClick={(e) => onClick(e, d.productId)}
    >

  ...
  return (
    <div className="h-full overflow-x-auto">
      <div className="min-w-[900px] h-full">
        {/*
          Use a CSS grid with 4 explicit rows (header, midi, soir, journée) × 7 cols.
          This ensures all rows are aligned across columns.
        */}
        <div
          className="grid h-full overflow-hidden rounded-xl border"
          style={{
        ...
        >
          ...
          {daysData.map(({ day, date, dayIsToday, dayIsPast, recap }) => (
            <div
              key={`header-${day}`}
              className={cn(
                "relative border-r border-b bg-card px-3 py-2 text-center",
          ...
          {daysData.map(({ day, dayIsPast, midiShifts, req }) => (
            <div
              key={`midi-${day}`}
              className={cn("border-r border-b bg-card px-2 py-2", dayIsPast && "opacity-50")}
            >

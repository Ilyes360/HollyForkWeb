  ...
  switch (status) {
    case "rupture":
      return impactedRecipeCount > 0
        ? { text: `Rupture \u2014 ${impactedRecipeCount} recette${impactedRecipeCount > 1 ? "s" : ""} impactee${impactedRecipeCount > 1 ? "s" : ""}`, variant: "danger" }
        : { text: "Rupture \u2014 aucune recette liee", variant: "danger" }
    case "stock_faible":
      return { text: "Stock faible \u2014 commande a prevoir", variant: "warning" }
    case "stock_ok":
      return { text: "Stock OK", variant: "success" }
    case "surstock":
      return { text: "Surstock \u2014 attention au gaspillage", variant: "warning" }
  }
}
...
export function getStockEmptyState(hasFilters: boolean): EmptyStateCopy {
  ...
  return {
    ...
    description: "Ajoute tes premiers produits \u2014 Holly Fork calcule les portions realisables.",
